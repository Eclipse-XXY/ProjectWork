...
<body>
	...
	<script type="text/javascript">
            $(function () {
			    ...
			    //这个查询的是第一页原因是因为pagination这个jQuery插件的查询是根据索引进行的查询不想我写的是根据页码进行查询
			    queryPage(0);
			    </c:if>
            ...
            function queryPage( pageIndex ){
            	var dataObj = {
        			"pageNo" : pageIndex+1 ,   //因为索引是0所以真正的页码需要加1 pageno 是属性名称,是否增加双引号无所谓
        			"pageSize" : 3
        		};
            	if(condition){
            		...
            		success : function(result){
            			...
            			if(result.success){
            				...
            				$.each(list,function(i,n){
            					...
            					content+="<tr>";
								content+="	<td>"+(i+1)+"</td>";
// 								因为checkbox的值给的是n.id所以我以前取到的都是对象的id
            				...
            				// 创建分页  num_entries表示总的条数
            			var num_entries = pageObj.totalSize ;
//             				给做分页显示的div容器绑定显示的数据样式
            				$("#Pagination").pagination(num_entries, {
            					num_edge_entries: 2, //边缘页数
            					num_display_entries: 4, //主体页数
            					callback: queryPage, //注意这个地方的回调函数不需要加（）pagination组件会通过函数名进行查找
            					items_per_page:pageObj.pageSize, //每页显示条数
            					current_page:(pageObj.pageNo-1), //当前页,索引从0开始 把当前页的索引拿到
            					prev_text:"上一页",//这个地方查看帮助文档 它的属性名不是随便起的需要和它提供的一样看帮助文档就好啦
            					next_text:"下一页"
            				}); 
